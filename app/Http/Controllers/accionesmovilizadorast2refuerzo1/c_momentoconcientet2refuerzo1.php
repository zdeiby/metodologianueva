...
namespace App\Http\Controllers\accionesmovilizadorast2refuerzo1;
...
class c_momentoconcientet2refuerzo1 extends Controller
{
...
// IR A LA VISTA BIENESTAR FISICO Y EMOCIONAL
    ...
    public function fc_momentoconcientet2refuerzo1(Request $request,$folio){
        if (!session('nombre')) {
         ...
         $herramientas = new m_herramientas();
            ...
            $informacion = DB::table($tabla)
             ...
             foreach ($informacion as $registro) {
                ...
                $selectedIdsT1 = DB::table('t1_momentoconciente')
                    ->where('folio', $encodedFolio)
                    ->where('linea', $lineat1)   // 200
                    ->where('paso',  $pasot1)    // 20020
                ...
                $selectedIdsT1R2 = DB::table('t1_momentoconciente')
                                    ->where('folio', $encodedFolio)
                                    ->where('linea', $lineat1r2)
                                    ->where('paso', $pasot1r2)
                                    ->pluck('momentoconciente')
                                    ->toArray();
                //$ids_excluir = array_merge($selectedIds, $selectedIdsT1R1,$selectedIdsT1);
                // Filtrar la lista para que no muestre los IDs seleccionados en el primer formulario
        


                $selectedIdsT1R3 = DB::table('t1_momentoconciente')
                    ->where('folio', $encodedFolio)
                    ->where('linea', $linea)
                    ->where('paso', $paso)
                    ->pluck('momentoconciente')
                    ->toArray();
                $ids_excluir = array_merge($selectedIds, $selectedIdsT1R1,$selectedIdsT1, $selectedIdsT1R2);
                // Filtrar la lista para que no muestre los IDs seleccionados en el primer formulario
                $listaT1R3 = DB::table('t_momentosconcientes')
                ...
                $momento_concientet1r3 = '';
                    foreach ($listaMomentos as $item) {
                        // ¿Ya fue seleccionado en visitas anteriores?
                        $seleccionadoAntes = in_array($item->id, $selectedIds) || in_array($item->id, $selectedIdsT1R1) || in_array($item->id, $selectedIdsT1R2) || in_array($item->id, $selectedIdsT1);
                        // ¿Seleccionado en la actual?
                        $seleccionadoActual = in_array($item->id, $selectedIdsT1R3);
        ...
        //fin t1r1
            ...
            return view('accionesmovilizadorast2refuerzo1/v_momentoconcientet2refuerzo1',  $datos,['variable'=>$folio,
                                                                    ...
                                                                    'foliomenu'=>$encodedFolio[0],
                                                                     ...
                                                                     'momento_concientet1r3'=>$momento_concientet1r3,
                                                                    ]);
    }

        public function fc_bienestarenfamiliat2refuerzo1(Request $request, $folio)
        {
            if (!session('nombre')) {
                return redirect()->route('login');
            }
        ...
        $datos['opcionesAccionMovilizadora'] = $opcionesAccionMovilizadora;
            ...
            return view('accionesmovilizadorast2refuerzo1/v_bienestarenfamiliat2refuerzo1', $datos, [
                'variable' => $folio,
                'folio' => $encodedFolio[0],
                'foliomenu' => $encodedFolio[0],
                'tabla' => $tabla,
          ...
          /*  public function fc_accionmovilizadoraqtt2refuerzo1(Request $request,$folio){
                if (!session('nombre')) {
                    // Si no existe la sesión 'usuario', redirigir al login
                    return redirect()->route('login');
                }
                 ...
                 $herramientas = new m_herramientas();
                   ...
                   // dd($informacionant);
                    ...
                    return view('accionesmovilizadorast2refuerzo1/v_accionmovilizadoraqtt2refuerzo1',  $datos,['variable'=>$folio,
                                                                            'folio'=>$encodedFolio[0],
                                                                            'foliomenu'=>$encodedFolio[0],
                                                                             'tabla'=>$tabla,
    ...
    public function fc_accionmovilizadoraqtt2refuerzo1(Request $request, $folio)
    {
        ...
        $accionesAnteriores = DB::table($tabla)
            ->where('folio', $encodedFolio)
            ->where('linea', '<', 600)   // 👈 solo las anteriores
            ->pluck('accionmovilizadora', 'bienestar')
            ->toArray();

        $bienestarant = $categoriasant[0]->categoria ?? null;
        $bienestar = $categorias[0]->categoria ?? null;
        ...
        // ---------------------------------------------------
        // 🔹 Traer TODAS las acciones seleccionadas para ese folio
        // ---------------------------------------------------
        $accionesSeleccionadas = DB::table($tabla)
            ->where('folio', $encodedFolio)
            ->pluck('accionmovilizadora', 'bienestar') // [bienestar_id => accionmovilizadora]
            ->toArray();

        // ---------------------------------------------------
        // 🔹 Obtener TODOS los bienestares
        // ---------------------------------------------------
        $todosBienestares = DB::table('t_bienestares')
            ->select('id', 'descripcion')
              ->where('id', '!=', 3) 
            ->orderBy('id', 'asc')
            ->get();

            $registroGuardado = DB::table($tabla)
                ->where('folio', $encodedFolio)
                ->where('linea', 600)
                ->where('paso', 60040)
                ->first();

            $datos['bienestar_guardado'] = $registroGuardado->bienestar ?? null;
            $datos['accion_guardada']   = $registroGuardado->accionmovilizadora ?? null;


            foreach ($todosBienestares as $b) {
                $acciones = $herramientas->m_leeracciones('t_accionesmovilizadoras', $b->id);

                // Acción de visitas anteriores (se pinta en azul)
                $accionUsada = $accionesAnteriores[$b->id] ?? null;

                if ($accionUsada) {
                    $acciones = preg_replace(
                        '/<option value="'.$accionUsada.'">(.*?)<\/option>/',
                        '<option value="'.$accionUsada.'" disabled style="color:blue;font-weight:bold">$1 (Seleccionada en visita anterior)</option>',
                        $acciones
                    );
                }
        ...
        $datos['todosBienestares'] = $todosBienestares;

        // ---------------------------------------------------
        // 🔹 Acciones del actual y anterior
        // ---------------------------------------------------
        if ($bienestar) {
            $datos['t_accionesmovilizadora'] = $herramientas->m_leeracciones('t_accionesmovilizadoras', $bienestar);
        }

        if ($bienestarant) {
            $datos['t_accionesmovilizadoraant'] = $herramientas->m_leeracciones('t_accionesmovilizadoras', $bienestarant);
        }
        ...
        // ---------------------------------------------------
        return view(
            'accionesmovilizadorast2refuerzo1/v_accionmovilizadoraqtt2refuerzo1',
            $datos,
            [
                'variable' => $folio,
                'folio' => $encodedFolio[0],
         ...
         /*   public function fc_accionmovilizadoracompromisost1refuerzo2(Request $request,$folio){
                if (!session('nombre')) {
                 ...
                 $herramientas = new m_herramientas();
                  ...
                  $datos['t_estados_compromisos'] = $herramientas->m_leert1('t_estados_compromisos');
                    ...
                    return view('accionesmovilizadorast1refuerzo3/v_accionmovilizadoracompromisost1refuerzo2',  $datos,['variable'=>$folio,
                                                                            'folio'=>$encodedFolio[0],
                                                                            'foliomenu'=>$encodedFolio[0],
                                                                            'tabla'=>$tabla,
                ...
                public function fc_accionmovilizadoracompromisost2refuerzo1(Request $request, $folio)
                {
                    if (!session('nombre')) {
                        return redirect()->route('login');
                    }
                    ...
                    $lineaAnteriorT1R2 = 400;
                    $pasoAnteriorT1R2 = 40040;

                    // Primera visita (anterior a la anterior)
                    $lineaAnteriorT1 = 200;
                    $pasoAnteriorT1 = 20040;

                     $lineaAnteriorT1R3 = 500; 
                     $pasoAnteriorT1R3 = 50040;
                    ...
                    $informacionantT1R1 = DB::table($tabla)
                        ...
                        $informacionantT1R2 = DB::table($tabla)
                        ->where('folio', $encodedFolio)
                        ->where('linea', $lineaAnteriorT1R2)
                        ->where('paso', $pasoAnteriorT1R2)
                        ->where('estado_compromiso', 3)
                        ->select('compromiso', 'numero_compromiso', 'estado_compromiso')
                        ->get();
                    ...
                    // --- CUARTA VISITA (línea 500 que pediste)
                    $informacionantT1R3 = DB::table($tabla)
                        ->where('folio', $encodedFolio)
                        ->where('linea', $lineaAnteriorT1R3)
                        ->where('paso', $pasoAnteriorT1R3)
                        ->select('compromiso', 'numero_compromiso', 'estado_compromiso')
                        ->get();
                    ...
                    foreach ($informacionantT1R2 as $c) {
                        $compromisosAnteriorTabla[] = [
                            'numero' => $c->numero_compromiso,
                            'compromiso' => $c->compromiso,
                            'estado' => $c->estado_compromiso,
                            'editable' => false,
                            'visita' => 'Tercera visita T1'
                        ];
                    }

                    foreach ($informacionantT1R3 as $c) {
                        $compromisosAnteriorTabla[] = [
                            'numero' => $c->numero_compromiso,
                            'compromiso' => $c->compromiso,
                            'estado' => $c->estado_compromiso,
                            'editable' => true,
                            'visita' => 'Cuarta visita T1'
                        ];
                    }
                    ...
                    return view('accionesmovilizadorast2refuerzo1/v_accionmovilizadoracompromisost2refuerzo1', $datos, [
                        'variable' => $folio,
                        'folio' => $encodedFolio[0],
                        'foliomenu' => $encodedFolio[0],
                        'tabla' => $tabla,
                        ...
                        'linea' => $linea,
                        'paso' => $paso,
                        'lineaanterior' => $lineaAnteriorT1R3,
                        'pasoanterior' => $pasoAnteriorT1R3,
                    ]);
                }
     ...
     public function fc_guardaraccionesmovilizadorast2refuerzo1(Request $request)
     {
         ...
         $now = Carbon::now();
         $data = $request->except(['folio', 'tabla','linea','paso','accionmovilizadora_5','accionmovilizadora_4','accionmovilizadora_2','accionmovilizadora_1','bienestar_seleccionado']);
     ...
     public function fc_agregarpasohogargeneral(Request $request){
      ...
      public function fc_verificarpasost2refuerzo1(Request $request)
      {
          $folio = $request->input('folio');
          $linea = $request->input('linea');
          $pasos = [ '60030', '60040']; // Pasos a verificar
          ...
          $resultado = DB::table('t1_pasosvisita')
              ...
              ->where('linea', $linea)
              ->whereIn('paso', $pasos)
              ->selectRaw('CASE WHEN COUNT(*) = 2 AND SUM(estado) = 2 THEN 1 ELSE 0 END AS resultado')
              ->value('resultado');
      ...
      public function fc_guardaraccionesmovilizadorascompromisost2refuerzo1(Request $request) {
