...
class c_cardsqt_visitas extends Controller
{
    public function fc_cardsqt(Request $request, $folio, $visita){
        if (!session('nombre')) {
      ...
      $decodeFolio = $hashids->decode($folio)[0];
     // dd($decodeFolio);
      $jefes=$modelo-> m_veredadrepjefe($decodeFolio);
    


      $foliobr=strval($decodeFolio);  

       //dd($foliobr, $visita);
      
      $metodologia = DB::table('t1_principalhogar')
      ->where('folio', $decodeFolio)
      ->value('metodologia');
    ...
    //
       // dd($decodeFolio);
        return view('v_cardsqt_visitas',$datos,["variable"=>$decodeFolio, "folioencriptado"=>$folio,'jefes' => $jefes, 'foliobycript'=>$foliobycript, 'indicadoreshogar'=>$indicadoreshogar,
